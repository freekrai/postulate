...
function js_string($input){
	...
	try {
		...
		foreach (str_split($wchars, 2) as $wchar) {
			...
			if ($o1 == 0 && ($o2 <= 0x1F || $wchar[1] == '<' || $wchar[1] == '>' || $wchar[1] == '"' || $wchar[1] == '&' )) {
...
function pluralize2($number, $noun, $nouns = false){
...
function summarize($text, $length, $ellipsis = '...', $encoding = 'UTF-8', $strict_cut = false, $strict_sentences = false){
	...
	$split_pos = ( $cut_str ? mb_strrpos($cut_str, $split_char, $encoding) : false );
